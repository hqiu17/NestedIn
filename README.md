...
There are quite a few phylogenetic tree sorting tools that are available. NestedIn is the only tool that targets 'nested position' and addresses the number of interior nodes that support query-donor monophyly. As outputs, NestedIn provides a list of tree names that meet uerser's search criteria and a directory containing the corresponidng tree files. If requested (argument `--ingroup`), it outputs, for each qualifying tree, the information of all the query-donor monophyletic clades found in the tree, including node support, donor sequences in the clade, and sequences of optional taxa that are allowed in the monophyletic clades (section 4.3.5).  
